...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Suhu Maksimum", 
        main = "Plot Deret Waktu Suhu Maksimum")
points(data1.ts)
...
Eksplorasi data dilakukan menggunakan package `ggplot2` dengan terlebih dahulu memanggil library *package* `ggplot2`.
...
ggplot() + 
  ...
  labs(x = "Periode", y = "Suhu Maksimum", color = "Legend") +
...
Pemulusan menggunakan metode SMA dilakukan dengan parameter `m=4`.
...
data.gab #forecast 30 periode ke depan
```

Adapun plot data deret waktu dari hasil peramalan yang dilakukan adalah sebagai berikut.

```{r}
ts.plot(data1.ts, xlab="Periode", ylab="Suhu Maksimum", main= "SMA N=4 Data Suhu maksimum")
points(data1.ts)
...
dilakukan peramalan untuk 30 periode kedepan menggunakan metode DMA
...
```{r}
ts.plot(data1.ts, xlab="Periode", ylab="Suhu Maksimum", main= "DMA N=4 Data Suhu Maksimum")
points(data1.ts)
...
Untuk mendapatkan gambar hasil pemulusan pada data latih dengan fungsi `ses()` , digunakan fungsi `autoplot()` dan `autolayer()` dari *library packages* `ggplot2` .
...
## Forecasting dengan Fungsi Holtwinters
